...
export default function SignupForm({ nextUrl }: { nextUrl: string }) {
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    router.push(`/login?signup=success&next=${encodeURIComponent(nextUrl)}`);
  };
  ...
  return (
    ...
    >
      ...
      <form className="mt-6 space-y-4" onSubmit={handleSubmit(onSubmit)}>
        ...
        <Button type="submit" loading={isSubmitting} className="w-full bg-primary text-white hover:bg-primary-light">
          Create account
        </Button>

        <p className="text-sm text-text-muted">
          Already have an account?{" "}
          <Link className="font-semibold text-primary-light" href={`/login?next=${encodeURIComponent(nextUrl)}`}>
            Log in
          </Link>
        </p>
      </form>
